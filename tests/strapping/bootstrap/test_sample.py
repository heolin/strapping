...
@pytest.mark.parametrize(
    "a1,a2,excepted,iterations,aggrfunc", [
        (
...
def test_sample_diffs(a1: np.ndarray, a2: np.ndarray, excepted: np.ndarray,
                      iterations: int, aggrfunc: Callable, freeze_random: None):
    result = sample_diffs(a1, a2, iterations=iterations, aggrfunc=aggrfunc)
    assert np.allclose(result, excepted)
    assert result.shape == (iterations * iterations, a1.shape[1])
